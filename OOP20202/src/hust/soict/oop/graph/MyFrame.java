...
public class MyFrame extends JFrame implements ActionListener {
	...
	private JButton btnRunAll, btnRunStep, btnRandom;
	...
	private void actionSave() {
		...
		if (select == 0) {
			String path = fc.getSelectedFile().getPath();
			System.out.println(path);
			BufferedImage image = new BufferedImage(myDraw.getWidth(), myDraw.getHeight(), BufferedImage.TYPE_INT_RGB);
			myDraw.paint(image.getGraphics());
			boolean saved = false;
			try {
				saved = ImageIO.write(image, "png", new File(path+ ".png"));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			if(saved) {
				JOptionPane.showMessageDialog(null, "Image file successfully saved!");
			}
			else {
	...
	private void showHelp() {
		if (frameHelp == null) {
			frameHelp = new HelpAndAbout(0, "Nhóm 5");
		}
	...
	private void showAbout() {
		if (frameAbout == null) {
			frameAbout = new HelpAndAbout(1, "Nhóm 5");
		}
